...
tab1 <- paste(tab$First, " (**", tab$Year, "**). ", tab$Title, ".", sep = "")

if (ut[j] != "Talks"){
  
# add status or journal
i1 <- (tab$Status == "s")
tab1 <- paste(tab1, " _", tab$Journal, "_", sep = "")
...
i2 <- ((tab$Status %in% c("s", "ta")) == FALSE & is.na(tab$Issue) == FALSE)
tab1[i2] <- paste(tab1, ", **", tab$Volume, "**(", tab$Issue, "), ", tab$Pages, ". ", sep = "")[i2]

# papers with reference and no issue
i2b <- ((tab$Status %in% c("s", "ta")) == FALSE & is.na(tab$Issue) == TRUE)
tab1[i2b] <- paste(tab1, ", **", tab$Volume, "**:", tab$Pages, ". ", sep = "")[i2b]
...
i4 <- (is.na(tab$preprint) == FALSE)
tab1[i4] <- paste(tab1, " | [slidedeck](", tab$preprint, ") | ", sep = "")[i4]

}



tab2 <- data.frame(cbind(tab1))
